...
- Channel-scoped Firestore documents (memories, quotes, timers, custom commands, command settings, auto-chat, language, game configs, personas) are keyed by the broadcaster's Twitch user ID, never by login name. Resolve the key with `channelDocKey(channelName)` from `src/lib/channelKey.js` (throws for a channel with no known ID rather than falling back to the name) and map a document back to a login with `channelNameForDocKey(docId, data)`. Store the login on the parent doc as `channelName` for readability only. The web UI (`chatsage-web-ui/functions/src/utils/channelKey.ts`) writes the same keys, so a change to the scheme lands in both repos together; `scripts/migrate-channel-keys.js` copies legacy login-keyed docs
