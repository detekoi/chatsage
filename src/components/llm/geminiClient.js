...
export async function generateStandardResponse(contextPrompt, userQuery) {
    ...
    const fullPrompt = `${contextPrompt}\n\nUSER: ${userQuery}\nREPLY: ≤300 chars. Prioritize substance; when helpful add a specific detail/fact/tip tied to the user's topic, and optionally a short, tailored question. No meta. Don't restate the question or context. Don't repeat the username. (Note: Attribute information to the current user based on their username in the chat history.)`;
...
export async function generateUnifiedResponse(contextPrompt, userQuery) {
    if (!userQuery?.trim()) return null;
    const model = getGeminiClient();
    const fullPrompt = `${contextPrompt}\n\nUSER: ${userQuery}\nREPLY: ≤320 chars, direct, grounded if needed. No meta. (Note: Attribute information to the current user based on their username in the chat history.)`;
    try {
        const result = await model.generateContent({
            contents: [{ role: 'user', parts: [{ text: fullPrompt }] }],
