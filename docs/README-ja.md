...
> 重要: 現在、ChatSage のクラウド版へのアクセスは招待制（許可リスト）です。未承認のチャンネル向けのセルフサービスダッシュボードは無効になっています。ボットを試したい場合は、こちらからご連絡ください: [お問い合わせフォーム](https://parfaitfair.com/#contact)。

**[ChatSageをあなたのTwitchチャンネルに追加 →](https://bot.wildcat.chat)**
...
注意: 許可リストにある承認済みチャンネルのみがChatSageを有効にできます。まだ承認されていないが試したい場合は、[お問い合わせフォーム](https://parfaitfair.com/#contact)からご連絡ください。

承認済みのチャンネルは、以下の手順でWebインターフェースから追加/削除できます。

1.  **ChatSage管理ポータルにアクセスします**：
    -   [ChatSage管理ポータル](https://bot.wildcat.chat) にアクセスします（承認済みチャンネルのみ）
