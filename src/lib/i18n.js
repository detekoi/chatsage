...
/**
 * A deferred message: carries the catalog key plus the ready-made English text.
 *
 * Callers hand this to enqueueMessage() instead of a string, letting ircSender resolve it once the
 * channel's language is known. It stringifies to the English fallback, so any code that
 * concatenates or measures it keeps working exactly as before.
 */
export class I18nMessage {
    constructor(key, params, fallback) {
        this.key = key;
        this.params = params || {};
        this.fallback = String(fallback ?? '');
    }
    toString() { return this.fallback; }
    get length() { return this.fallback.length; }
}

/**
 * Builds a deferred message for enqueueMessage().
 ...
 */
export function msg(key, params, fallback) {
    return new I18nMessage(key, params, fallback);
}

/**
 * Resolves a value that may be an I18nMessage or a plain string into a string for the given language.
 * @param {I18nMessage|string} value
 * @param {string|null} language
 * @returns {string}
 */
export function resolve(value, language = null) {
    if (value instanceof I18nMessage) {
        return t(value.key, value.params, language) ?? value.fallback;
    }
    return String(value ?? '');
}
