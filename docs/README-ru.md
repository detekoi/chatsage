...
Стримеры теперь могут легко добавлять или удалять ChatSage со своего канала с помощью веб-интерфейса:

1. **Посетите портал управления ChatSage**:
   - Перейдите на [Портал управления ChatSage](https://streamsage-bot.web.app)
...
4. **Время присоединения бота**:
   ...
   - Важно: если бот не отвечает, предоставьте ему статус модератора командой `/mod StreamSageTheBot`

5. **Взаимодействие с пользователем**:
   - Зрители могут взаимодействовать с ChatSage, упоминая его: `@StreamSageTheBot привет` (имя пользователя будет обновлено, чтобы отразить новое имя, ChatSage, когда Twitch позволит мне это сделать)
