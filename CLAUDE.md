...
- Fixed user-facing chat strings belong in `src/locales/en.js` and are looked up with `t()` from `src/lib/i18n.js`, which returns `null` for uncatalogued languages so the English fallback still reaches the runtime translator. After editing `en.js`, run `npm run i18n:translate`. Dynamic text (LLM replies, generated game content) keeps using `translationUtils.js` as before
